...
# Telegram bot token (replace with your own or use env var)
BOT_TOKEN = os.environ.get("BOT_TOKEN", "8796116897:AAGvuuOWeIuzcp1xjHfj9Aq-mqB-vkMV1eg")
bot = telebot.TeleBot(BOT_TOKEN, threaded=False)

# Owner and user data files
OWNER_FILE = "owner_data.json"
BANNED_FILE = "banned_users.json"
USERS_DB = "users_db.json"
ALLOWED_OWNERS = [7637620066, 8348660678]  # Replace with your Telegram IDs
...
def start_server(username, server_name):
    try:
        ...
        def monitor_process(proc, key):
            ...
            cfg = load_server_config(username, server_name)
            cfg['status'] = 'stopped'
            cfg.pop('pid', None)
            save_server_config(username, server_name, cfg)
        threading.Thread(target=monitor_process, args=(p, (username, server_name)), daemon=True).start()
        return True
    except Exception as e:
...
def auto_fix_and_restart(name):
    ...
    # 6. If still broken, replace with working test server
...
def ask_ai():
    # Simple AI response (replace with real LLM if desired)
